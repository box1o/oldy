...
foreach(language IN ITEMS c cpp)
    ...
    if(language STREQUAL "cpp")
        if(NOT EXISTS "${project_dir}/src/extension.cpp")
            message(FATAL_ERROR "Generated C++ project did not use the external C++ template")
        endif()
        file(READ "${project_dir}/src/extension.cpp" cpp_source)
        if(NOT cpp_source MATCHES "#include <woki/extension.hpp>" OR
           NOT cpp_source MATCHES "WOKI_EXTENSION\\(Extension\\)" OR
           NOT cpp_source MATCHES "slog::Info" OR
           cpp_source MATCHES "Context")
            message(FATAL_ERROR "Generated C++ project does not use the component extension facade")
        endif()
        set(build_config Debug)
        set(expected_config_flag "-O0")
    else()
        if(NOT EXISTS "${project_dir}/src/extension.c")
            message(FATAL_ERROR "Generated C project did not use the external raw C template")
        endif()
        file(READ "${project_dir}/src/extension.c" c_source)
        if(NOT c_source MATCHES "ext_api_version" OR NOT c_source MATCHES "ext_on_command")
            message(FATAL_ERROR "Generated C project lost the advanced raw ABI scaffold")
        endif()
    ...
    if(language STREQUAL "cpp")
        ...
        set(source_file "${project_dir}/src/extension.cpp")
        set(source_backup "${TEST_ROOT}/extension-backup.cpp")
