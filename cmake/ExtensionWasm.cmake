...
function(add_wokiext target)
    ...
    cmake_parse_arguments(WOKIEXT "" "LANGUAGE;MANIFEST;WASM" "SOURCES;ASSETS;LIBRARIES;PERMISSIONS" ${ARGN})
    ...
    if(WOKIEXT_WASM)
        set(_wasm_relative "${WOKIEXT_WASM}")
        set(_guest_libraries ${WOKIEXT_LIBRARIES})
        set(_permissions ${WOKIEXT_PERMISSIONS})
    else()
        message(WARNING "add_wokiext(${target}) is using deprecated configure-time manifest extraction; pass WASM, LIBRARIES, and PERMISSIONS explicitly")
    ...
    if(TARGET woki-config)
        set(_manifest_validator $<TARGET_FILE:woki-config>)
    else()
        find_program(_manifest_validator NAMES woki-config)
    endif()
    ...
    if(_manifest_validator)
        if(TARGET woki-config)
            add_dependencies(${target} woki-config)
        endif()
        add_custom_command(TARGET ${target} PRE_LINK
            COMMAND ${_manifest_validator} validate "${_manifest}" extension.manifest@1
            VERBATIM
        )
    else()
        message(WARNING "woki-config was not found; manifest validation is deferred to package load")
    endif()
