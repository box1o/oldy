...
public:
    ...
    [[nodiscard]] Result<BorrowedShader> BorrowProduct(ContentHash product, u64 generation) const;
    [[nodiscard]] std::optional<ShaderAssetRecord> Record(ShaderAssetHandle handle) const;
    void MarkUsed(const BorrowedShader& shader, rhi::SubmissionTicket submission);
    [[nodiscard]] size_t PruneModules(DeferredReleaseQueue& releases);
