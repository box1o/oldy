...
public:
    ...
    [[nodiscard]] ComposedSource Compose(
        const ShaderDescriptor& descriptor,
        std::optional<asset::AssetPath> descriptor_path = std::nullopt
    ) const;
...
[[nodiscard]] Result<ref<rhi::ShaderModule>> CreateShaderModuleFromAsset(
    rhi::Device& device,
    const asset::Vfs& vfs,
    const asset::AssetPath& descriptor_path
);
#endif
