fn pack_unorm4x8_rgba(value: vec4f) -> u32 {
    return pack4x8unorm(clamp(value, vec4f(0.0), vec4f(1.0)));
}

fn unpack_unorm4x8_rgba(value: u32) -> vec4f {
...
fn encode_octahedral(normal: vec3f) -> vec2f {
    ...
    if (projected.z < 0.0) {
        projected = vec3f(
            (vec2f(1.0) - abs(projected.yx)) * sign(projected.xy),
            projected.z
        );
    }
...
fn decode_octahedral(encoded: vec2f) -> vec3f {
    ...
    if (normal.z < 0.0) {
        normal = vec3f(
            (vec2f(1.0) - abs(normal.yx)) * sign(normal.xy),
            normal.z
        );
    }
