...
fn validate_compute(@builtin(global_invocation_id) global_id: vec3u) {
    if (!dispatch_in_bounds(global_id, validation.dimensions)) {
        return;
    }
    let index = global_linear_index(global_id, validation.dimensions);
    if (index < validation.element_count) {
        validation_output[index] = index;
    }
}
