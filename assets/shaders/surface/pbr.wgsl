...
fn fresnel_schlick(cos_theta: f32, f0: vec3f) -> vec3f {
    return f0 + (vec3f(1.0) - f0) * pow5(1.0 - saturate(cos_theta));
}
...
fn evaluate_pbr_brdf(
    ...
    let specular = distribution_ggx(n_dot_h, max(surface.roughness, 0.045))
        * visibility_smith_ggx_correlated(
            n_dot_v,
            n_dot_l,
            max(surface.roughness, 0.045)
        )
        * fresnel;
